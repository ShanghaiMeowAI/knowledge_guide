...
`knowledge_guide` 使用 Odoo 标准翻译机制。页面标题、分类、正文 HTML、合集名称、合集简介和跳转按钮文字都已经定义为可翻译字段。

语言显示规则：

- 后台阅读指南时，默认使用当前登录用户的语言。
- 后台左侧提供语言下拉框，用户可以临时切换到其他已启用语言。
- 公开指南默认使用访问用户对应的 Odoo 语言上下文。
- 公开指南侧边栏提供语言下拉框，切换后 URL 会带上 `lang=<语言代码>`，翻页时会继续保留该语言。
- 只有 Odoo 中已启用的语言会出现在语言下拉框里。

维护翻译内容时，可以在 Odoo 后台为 `knowledge.guide.book`、`knowledge.guide.page` 和 `knowledge.guide.action.link` 的可翻译字段补充译文。业务定制模块仍然只维护默认语言内容；其他语言通过 Odoo 翻译记录维护，不需要为每种语言复制一套指南页面。
