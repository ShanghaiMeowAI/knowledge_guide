...
class TestKnowledgeGuidePage(TransactionCase):
    ...
    def test_cleanup_legacy_sample_pages_archives_only_old_templates(self):
        """Old English sample guide records are archived without touching user pages."""
        legacy_page = self.Page.create({
            'name': 'Welcome',
            'category': 'GETTING STARTED',
            'content_html': '<p>Welcome to your knowledge guide</p>',
            'module_source': 'knowledge_guide',
        })
        user_page = self.Page.create({
            'name': 'Welcome',
            'category': 'General',
            'content_html': '<p>Company onboarding notes</p>',
        })

        self.Page._cleanup_legacy_sample_pages()
